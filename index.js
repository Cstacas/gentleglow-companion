  ...
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (customId === 'plant_dash_water') {
        ...
        dbGrowPlant(user.id, 25);
        
        const updatedPlant = dbGetUserPlant(user.id);
        let responseText = `💧 **Watered**: You watered your plant! It gained **25 EXP**!`;
        
        const styleClean = plant.pot_style.toLowerCase();
        if (styleClean === 'terracotta') {
          responseText += `\n🪵 *The water quickly soaks into the rustic terracotta clay.*`;
        } else if (styleClean === 'ceramic') {
          responseText += `\n☕ *The ceramic mug fills with clean water, keeping the roots cozy.*`;
        } else if (styleClean === 'glass') {
          responseText += `\n💎 *Droplets run down the sides of the shining glass vase, catching the light.*`;
        } else if (styleClean === 'marble') {
          responseText += `\n🪨 *The polished marble container holds the moisture perfectly.*`;
        } else if (styleClean === 'wood') {
        ...
        if (updatedPlant.stage > plant.stage) {
          const stageNames = ['Sprout 🌱', 'Bud 🌿', 'Bloom 🌸'];
          responseText += `\n\n🎉 **Stage Up!** Your plant has grown into a **${stageNames[updatedPlant.stage] || 'beautiful plant'}**!`;
        }
      ...
      if (customId === 'plant_dash_seed_trigger') {
        const selectMenu = new StringSelectMenuBuilder()
          ...
          .addOptions([
            ...
            { label: 'Red Tulip 🌷', value: 'tulip' },
            { label: 'Pink Rose 🌹', value: 'pink_rose' },
            { label: 'Bluebells 🔔', value: 'bluebells' },
            { label: 'Blue Daisy 🌼', value: 'blue_daisy' },
            { label: 'White Lily ⚜️', value: 'white_lily' },
            { label: 'Little White Flowers 💮', value: 'white_flowers' },
            { label: 'Tropical Hibiscus 🌺', value: 'tropical' },
            { label: 'Desert Cactus 🌵', value: 'cactus' },
      ...
      if (customId === 'plant_dash_customize') {
        ...
        const potMenu = new StringSelectMenuBuilder()
          ...
          .addOptions([
            ...
            { label: 'Cozy Ceramic Mug ☕', value: 'ceramic' },
            { label: 'Polished Marble Pot 🪨', value: 'marble' },
            { label: 'Rustic Wooden Planter 📦', value: 'wood' }
          ]);
...
function getPlantGif(plant, action = 'idle') {
  return 'https://media.giphy.com/media/3o7abKhOpu0NXS3l4I/giphy.gif'; // Universal pixel watering animation
}

function buildPlantEmbed(plant, action = 'idle') {
  const stages = ['Sprout 🌱', 'Bud 🌿', 'Bloom 🌸'];
  const ASCII_ART = [
    '[ | ] (A healthy green sprout)',
    '  \\|/  \n   |    (A budding plant with leaves)',
    ' _(_)_ \n(_)*(_) \n  \\|/  \n   |    (A beautiful blooming flower!)'
  ];

  const pots = {
    terracotta: "Terracotta Clay Pot 🪵",
    ceramic: "Cozy Ceramic Mug ☕",
    glass: "Shining Glass Vase 💎",
    marble: "Polished Marble Pot 🪨",
    wood: "Rustic Wooden Planter 📦"
  };
  ...
  const plantNames = {
    white_flowers: 'Little White Flowers 💮',
    bluebells: 'Bluebells 🔔',
    tulip: 'Red Tulip 🌷',
    blue_daisy: 'Blue Daisy 🌼',
    cactus: 'Desert Cactus 🌵',
    sunflower: 'Cheerful Sunflower 🌻',
    fern: 'Cozy Fern 🌿',
    pink_rose: 'Pink Rose 🌹',
    white_lily: 'White Lily ⚜️',
    tropical: 'Tropical Hibiscus 🌺'
  };

  const stageName = stages[plant.stage] || 'Sprout 🌱';
  const art = ASCII_ART[plant.stage] || ASCII_ART[0];
  
  const potName = pots[plant.pot_style] || pots.terracotta;
  const decorName = decors[plant.decor] || decors.none;
  const breedName = plantNames[plant.plant_type] || plant.plant_type;
  
  let nextStageText = '';
  const currentWaterDays = plant.water_days || 0;
  if (plant.stage === 0) {
    nextStageText = ` (${10 - currentWaterDays} more watering days to Bud)`;
  } else if (plant.stage === 1) {
    nextStageText = ` (${20 - currentWaterDays} more watering days to Bloom)`;
  } else {
    nextStageText = ` (Fully Bloomed! 🎉)`;
  }
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      `**Watering Days**: \`${currentWaterDays}\` days${nextStageText}\n` +
      `**Experience Points**: \`${plant.experience}\` EXP\n\n` +
      `**Visual Status**:\n\`\`\`\n${art}\n\`\`\`\n` +
      `*Water your plant daily to watch it grow from a sprout into a full bloom!*`
    )
    .setColor('#4caf50')
    .setImage(gifUrl)
    .setTimestamp();
