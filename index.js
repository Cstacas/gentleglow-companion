  ...
  try {
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (customId === 'pet_dash_change_type_trigger') {
        console.log("[DEBUG] Trigger button clicked - step 1: Querying DB");
        const pet = dbGetUserPet(user.id);
        if (!pet) {
          console.log("[DEBUG] No pet found for user");
          return await interaction.reply({ content: '🐾 Adopt a pet first!', ephemeral: true });
        }

        console.log("[DEBUG] Step 2: Deferring update");
        await interaction.deferUpdate();
        console.log("[DEBUG] Step 3: Defer update complete. Building components");
