  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === 'plant') {
        ...
        const plantUrl = `${COMPANION_BASE_URL}/?name=${encodeURIComponent(plant.plant_type)}&type=plant&breed=${plant.plant_type}&acc=${plant.decor}&love=${plant.experience}&pot=${plant.pot_style}`;
        const row = new ActionRowBuilder().addComponents(
      ...
      if (commandName === 'pet') {
        ...
        const petUrl = `${COMPANION_BASE_URL}/?name=${encodeURIComponent(pet.pet_name)}&type=${pet.pet_type}&breed=${pet.breed}&acc=${pet.accessory}&love=${pet.love}`;
        const row = new ActionRowBuilder().addComponents(
    ...
    if (interaction.isButton()) {
      ...
      if (customId === 'plant_dash_water') {
        ...
        const plantUrl = `${COMPANION_BASE_URL}/?name=${encodeURIComponent(updatedPlant.plant_type)}&type=plant&breed=${updatedPlant.plant_type}&acc=${updatedPlant.decor}&love=${updatedPlant.experience}&pot=${updatedPlant.pot_style}`;
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('plant_dash_water').setLabel('Water Plant 💦').setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId('plant_dash_seed_trigger').setLabel('Re-Seed Plant 🌱').setStyle(ButtonStyle.Secondary),
          new ButtonBuilder().setCustomId('plant_dash_customize').setLabel('Customize 🎨').setStyle(ButtonStyle.Secondary),
      ...
      if (customId === 'pet_dash_pet' || customId === 'pet_dash_feed' || customId === 'pet_dash_play') {
        ...
        if (action === 'pet') {
          ...
          } else if (pet.accessory === 'glasses') {
            responseText = `🐾 You pet **${pet.pet_name}**. They look extremely cool looking back at you through their **Sunglasses 😎**! (Friendship +5 ❤️)`;
          ...
          if (pet.accessory === 'bowtie') {
            ...
            responseText = `🦴 You feed **${pet.pet_name}** treats. They catch them out of the air looking like an absolute boss in their **Sunglasses 😎**! (Friendship +5 ❤️)`;
          } else {
          ...
          if (pet.accessory === 'party') {
            ...
            responseText = `⚽ You play fetch! **${pet.pet_name}** chases the ball and catches it without their **Sunglasses 😎** falling off! (Friendship +5 ❤️)`;
          } else if (pet.accessory === 'scarf') {
        ...
        const petUrl = `${COMPANION_BASE_URL}/?name=${encodeURIComponent(updatedPet.pet_name)}&type=${updatedPet.pet_type}&breed=${updatedPet.breed}&acc=${updatedPet.accessory}&love=${updatedPet.love}`;
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('pet_dash_pet').setLabel('Pet 🐾').setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId('pet_dash_feed').setLabel('Feed Treats 🦴').setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId('pet_dash_play').setLabel('Play Fetch ⚽').setStyle(ButtonStyle.Primary),
      ...
      if (customId === 'pet_dash_customize') {
        ...
        const accMenu = new StringSelectMenuBuilder()
          ...
          .addOptions([
            ...
            { label: 'Cool Sunglasses 😎', value: 'glasses' },
