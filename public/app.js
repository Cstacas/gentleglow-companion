...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Plant Action Overlay Bindings
  const wateringCan = document.getElementById('watering-can');
  const scissors = document.getElementById('scissors');
  const plantChatBubble = document.getElementById('plant-chat-bubble');
      ...
      try {
        if (type === 'plant') {
          ...
          if (data) {
            ...
            state.stage = data.stage || 0; // Load the growth stage!
            ...
            if (lcdScreen) lcdScreen.classList.add('plant-mode');
  ...
  if (type === 'plant') {
    ...
    if (spriteWrapper) spriteWrapper.style.display = 'flex';
    if (plantContainer) plantContainer.style.display = 'none';
    if (lcdScreen) lcdScreen.classList.remove('plant-mode');
  }
  ...
  const PLANT_ANIMS = {
    white_flowers: {
      atlas: '18x32_plantanimation_atlas.png',
      frameWidth: 18, frameHeight: 32, row: 0, atlasWidth: 414, atlasHeight: 160,
      stages: {
        0: { start: 1, end: 3, speed: 200 }, // Sprout
        1: { start: 4, end: 6, speed: 200 }, // Bud
        2: { start: 8, end: 11, speed: 150 } // Bloom
      }
    },
    bluebells: {
      atlas: '18x32_plantanimation_atlas.png',
      frameWidth: 18, frameHeight: 32, row: 1, atlasWidth: 414, atlasHeight: 160,
      stages: {
        0: { start: 1, end: 3, speed: 200 },
        1: { start: 4, end: 6, speed: 200 },
        2: { start: 8, end: 11, speed: 150 }
      }
    },
    tulip: {
      atlas: '18x32_plantanimation_atlas.png',
      frameWidth: 18, frameHeight: 32, row: 2, atlasWidth: 414, atlasHeight: 160,
      stages: {
        0: { start: 1, end: 3, speed: 200 },
        1: { start: 4, end: 6, speed: 200 },
        2: { start: 8, end: 11, speed: 150 }
    ...
    blue_daisy: {
      atlas: '18x32_plantanimation_atlas.png',
      frameWidth: 18, frameHeight: 32, row: 3, atlasWidth: 414, atlasHeight: 160,
      stages: {
        0: { start: 1, end: 3, speed: 200 },
        1: { start: 4, end: 7, speed: 200 },
        2: { start: 10, end: 14, speed: 150 }
      }
    },
    cactus: {
      atlas: '18x32_plantanimation_atlas.png',
      frameWidth: 18, frameHeight: 32, row: 4, atlasWidth: 414, atlasHeight: 160,
      stages: {
        0: { start: 1, end: 3, speed: 250 },
        1: { start: 4, end: 6, speed: 250 },
        2: { start: 9, end: 12, speed: 200 }
      }
    },
    sunflower: {
      atlas: '32x48_plantanimation_atlas.png',
      frameWidth: 32, frameHeight: 48, row: 0, atlasWidth: 1824, atlasHeight: 192,
      stages: {
        0: { start: 1, end: 5, speed: 150 },
        1: { start: 9, end: 13, speed: 150 },
        2: { start: 24, end: 56, speed: 120 }
      }
    },
    fern: {
      atlas: '32x48_plantanimation_atlas.png',
      frameWidth: 32, frameHeight: 48, row: 1, atlasWidth: 1824, atlasHeight: 192,
      stages: {
        0: { start: 1, end: 4, speed: 200 },
        1: { start: 8, end: 12, speed: 200 },
        2: { start: 15, end: 23, speed: 150 }
      }
    },
    pink_rose: {
      atlas: '32x48_plantanimation_atlas.png',
      frameWidth: 32, frameHeight: 48, row: 2, atlasWidth: 1824, atlasHeight: 192,
      stages: {
        0: { start: 2, end: 5, speed: 200 },
        1: { start: 9, end: 13, speed: 200 },
        2: { start: 20, end: 35, speed: 120 }
      }
    },
    white_lily: {
      atlas: '32x48_plantanimation_atlas.png',
      frameWidth: 32, frameHeight: 48, row: 3, atlasWidth: 1824, atlasHeight: 192,
      stages: {
        0: { start: 2, end: 5, speed: 200 },
        1: { start: 8, end: 12, speed: 200 },
        2: { start: 18, end: 34, speed: 120 }
      }
    },
    tropical: {
      atlas: '48x32_plantanimation_atlas.png',
      frameWidth: 48, frameHeight: 32, row: 0, atlasWidth: 1152, atlasHeight: 32,
      stages: {
        0: { start: 1, end: 4, speed: 200 },
        1: { start: 8, end: 12, speed: 200 },
        2: { start: 13, end: 23, speed: 150 }
      }
    }
  };
  ...
  function renderSprite(actionKey = 'idle') {
    if (type === 'plant') {
      ...
      const plantConfig = PLANT_ANIMS[breedClean] || PLANT_ANIMS['sunflower'];
      const stage = state.stage || 0;
      const stageConfig = plantConfig.stages[stage] || plantConfig.stages[0];
      
      const frameWidth = plantConfig.frameWidth;
      const frameHeight = plantConfig.frameHeight;
      const row = plantConfig.row;
      const scale = 3.5; // Upscale pixel art
      
      plantSprite.style.width = `${frameWidth * scale}px`;
      plantSprite.style.height = `${frameHeight * scale}px`;
      plantSprite.style.backgroundImage = `url('images/${plantConfig.atlas}')`;
      plantSprite.style.backgroundSize = `${plantConfig.atlasWidth * scale}px ${plantConfig.atlasHeight * scale}px`;
      plantSprite.style.margin = '0 auto';
      plantSprite.style.marginBottom = '-15px'; // Sink slightly into pot
      
      let currentFrame = stageConfig.start;
      const updateFrame = () => {
        const leftPos = -(currentFrame * frameWidth * scale);
        const topPos = -(row * frameHeight * scale);
        plantSprite.style.backgroundPosition = `${leftPos}px ${topPos}px`;
        
        currentFrame++;
        if (currentFrame > stageConfig.end) {
          currentFrame = stageConfig.start;
        }
      };
      
      updateFrame();
      plantInterval = setInterval(updateFrame, stageConfig.speed);
      
      // Render pot dynamically from flowerpot_atlas.png
      if (plantPot) {
        const potWidth = 32;
        const potHeight = 32;
        const potScale = 3.5;
        plantPot.style.width = `${potWidth * potScale}px`;
        plantPot.style.height = `${potHeight * potScale}px`;
        plantPot.style.backgroundImage = "url('images/flowerpot_atlas.png')";
        plantPot.style.backgroundSize = `${224 * potScale}px ${160 * potScale}px`;
        
        // Map potStyle to columns: terracotta = 0, ceramic = 1, glass = 2, marble = 3, wood = 4
        let potCol = 0;
        const styleClean = potStyle.toLowerCase();
        if (styleClean === 'ceramic') potCol = 1;
        else if (styleClean === 'glass') potCol = 2;
        else if (styleClean === 'marble') potCol = 3;
        else if (styleClean === 'wood') potCol = 4;
        
        plantPot.style.backgroundPosition = `-${potCol * potWidth * potScale}px 0px`;
      }
      
      if (plantDecor) {
        plantDecor.className = `plant-decor-layer ${accessory}`;
      }
    } else {
      if (plantInterval) {
        clearInterval(plantInterval);
      ...
      const pTypeClass = type.toLowerCase();
      if (state.isSleeping) {
        spriteImage.className = 'pet-sprite-sheet sleep';
        if (petCharacter) petCharacter.className = `pet-character-container sleep ${pTypeClass}`;
        if (sleepZzz) sleepZzz.classList.add('active');
      } else if (actionKey === 'walk') {
        spriteImage.className = 'pet-sprite-sheet walk';
        if (petCharacter) petCharacter.className = `pet-character-container walk ${pTypeClass}`;
        if (sleepZzz) sleepZzz.classList.remove('active');
      } else {
        spriteImage.className = 'pet-sprite-sheet idle';
        if (petCharacter) petCharacter.className = `pet-character-container idle ${pTypeClass}`;
        if (sleepZzz) sleepZzz.classList.remove('active');
      }
      ...
      if (accessoryOverlay) {
        if (accessory !== 'none') {
          ...
          if (pType === 'dog') {
            if (breedClean === 'corgi') {
              ...
              if (accessory === 'party' || accessory === 'detective') {
                ...
                leftVal = '70%'; // Shifted forward to neck
                topVal = '104px';  // Lowered further to place on neck instead of face
              }
            }
          } else if (pType === 'cat') {
            if (breedClean === 'egypt') {
              ...
              if (accessory === 'party' || accessory === 'detective') {
                ...
                topVal = accessory === 'party' ? '30px' : '39px'; // Adjusted to fit snugly
              } else if (accessory === 'glasses') {
                leftVal = '75%'; // Aligned horizontally with eyes
                topVal = '64px'; // Aligned vertically with eyes
              } else {
                leftVal = '70%'; // Right facing neck
                topVal = '94px'; // Lowered another 8px to prevent breathing bob overlap
              }
            } else if (breedClean === 'space') {
              // Space cats have a bulky space helmet
              if (accessory === 'party' || accessory === 'detective') {
                leftVal = '72%'; // Right facing head
                topVal = accessory === 'party' ? '34px' : '43px'; // Adjusted to fit snugly
              } else if (accessory === 'glasses') {
                leftVal = '75%'; // Aligned horizontally with eyes
                topVal = '72px'; // Aligned vertically with eyes
              } else {
                leftVal = '70%'; // Right facing neck
                topVal = '104px'; // Lowered another 8px to prevent breathing bob overlap
              }
            } else {
              // Standard cats (standard, calico, tiger, batman, retro, pixel) face right
              if (accessory === 'party' || accessory === 'detective') {
                leftVal = '72%'; // Right facing head
                topVal = accessory === 'party' ? '38px' : '47px'; // Adjusted to fit snugly
              } else if (accessory === 'glasses') {
                leftVal = '75%', topVal = '74px'; // Aligned horizontally and vertically with eyes
              } else {
                leftVal = '70%'; // Right facing neck
                topVal = '102px'; // Lowered another 8px to prevent breathing bob overlap
              }
            }
          ...
          // Force browser to restart the accessory CSS keyframe animation from 0s
          // to keep it perfectly synchronized in phase with the GIF frame loop
          accessoryOverlay.style.animation = 'none';
          void accessoryOverlay.offsetWidth; // trigger reflow
          accessoryOverlay.style.animation = '';
        } else {
...
function stopWandering() {
  ...
  function triggerAction(actionName, emojiOverlay, actionMoodText, actionKey) {
    ...
    if (actionName === 'water' && type === 'plant') {
      if (wateringCan) wateringCan.classList.add('active');
    } else if (actionName === 'prune' && type === 'plant') {
      if (scissors) scissors.classList.add('active');
    } else if (actionName === 'talk' && type === 'plant') {
      if (plantChatBubble) {
        const phrases = ["Hello! 🌱", "Sunny day! ☀️", "Growing... ✨", "Thank you! ❤️", "Happy roots! 🌸", "Breeze is nice 🍃", "Cozy! 🥰", "Grateful ✨"];
        const randomPhrase = phrases[Math.floor(Math.random() * phrases.length)];
        const textSpan = plantChatBubble.querySelector('.chat-text');
        if (textSpan) textSpan.textContent = randomPhrase;
        plantChatBubble.classList.add('active');
      }
    }
    ...
    setTimeout(() => {
      ...
      if (scissors) scissors.classList.remove('active');
      if (plantChatBubble) plantChatBubble.classList.remove('active');
