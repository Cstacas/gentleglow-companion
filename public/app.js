    ...
    if (type === 'plant') {
      ...
      if (accessoryOverlay) {
        if (accessory !== 'none') {
          ...
          // Apply dynamic horizontal offsets based on pet type and face direction
          let leftVal = '50%';
          const breedClean = breed.toLowerCase();
          const isDogOrFox = (type === 'dog' && breedClean !== 'pug') || type === 'fox';
          
          if (type === 'dog' || type === 'fox') {
            if (breedClean === 'pug') {
              // Pugs are front-facing/centered!
              leftVal = '50%';
            } else {
              // Standard dogs and foxes face right (offset head/neck to the right side)
              if (accessory === 'party' || accessory === 'detective') {
                leftVal = '72%'; // head top-right
              } else if (accessory === 'glasses') {
                leftVal = '75%'; // eyes top-right
              } else {
                leftVal = '64%'; // neck/bowtie mid-right
              }
            }
          } else if (type === 'cat' || type === 'rabbit' || type === 'parrot' || type === 'turtle') {
            // Cats, rabbits, parrots, turtles face left (offset head/neck to the left side)
            if (accessory === 'party' || accessory === 'detective') {
              leftVal = '28%'; // head top-left
            } else if (accessory === 'glasses') {
              leftVal = '25%'; // eyes top-left
            } else {
              leftVal = '36%'; // neck/bowtie mid-left
            }
          } else {
            // Pandas, ghosts, and front-facing companions remain centered
            leftVal = '50%';
          }
